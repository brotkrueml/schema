...
The special attributes are starting with a dash (-) to separate them from the
common properties of the schema.org specification and to avoid collisions. Let's
have a deeper look on them.
...
you can see that the corporation type view helper uses -as to connect to the
``worksFor`` property of the person type view helper.
...
is not very convenient to use a switch or if view helper to choose the correct
type. For this scenario you can benefit from this argument:
...
.. important::
   ...
   When using the ``-specificType`` attribute you can only set the properties of
   the original type view helper, no additional ones from the specific type.
...
You can only set a string value in the argument of a type view helper, but
...
You know already the :html:`-as` attribute from the type view helpers. Its
purpose is the same, it references the property in the parent
:html:`<schema:type>` view helper.
...
:ref:`The Breadcrumb Markup <breadcrumb-view-helper>`.
